...
def scanner(file_list):
    for filename in file_list:
        ...
        while contents:
            ...
            exec_keyword(kwds, keyword, args)

def exec_keyword(kwds, keyword, args):
    if keyword in kwds.keys():
        kwds[keyword].parse_input(args)
    else:
...
class Named_List_with_descriptions(Named_List):
    ...
    def tex_print(self, show_name : bool):
        out = ''
        if show_name:
            out += f'\\subsection*{{{self.name}}}\n'
        
        described = list(filter(lambda x : x, self))
...
kwds = { k : v for k, v in globals().items() if type(v) in
        ( Ability
        , Skill
        , Class_Level
        , Named_String
        , Movement
        , Armorclass
        , Named_List_with_descriptions
        , Named_List_compact
        , Maybe_Int
        ) }

#multiline keywords are precisely the lists.
mkwds = { k for k, v in kwds.items() if issubclass(type(v), Named_List) }

expandable = {
        (k + 'Mod') : v.get_modifier
        for k, v in kwds.items() if type(v) is Ability
        }
